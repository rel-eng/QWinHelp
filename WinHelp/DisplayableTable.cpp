/* Displayable table class implementation.
...
{
    PRINT_DBG("Displayable table default constructor");
}

DisplayableTable::DisplayableTable(const void *src,
    size_t srcSize,
    const void *textSrc,
    size_t textSize,
    QTextCodec *codec,
    ...
    topicDescriptorNumber, topicNumber), columns(), paragraphs(), texts()
{
    ...
    PRINT_DBG("Loading displayable table");
    if((srcSize > static_cast<size_t>(0)) && (src != NULL))
    {
        ...
        PRINT_DBG("        Number of columns: %d", this->numberOfColumns);
        checkOffsetLengthSize(offset, static_cast<size_t>(1), srcSize);
        this->tableType =
            getUnsignedByte(reinterpret_cast<const void *>(reinterpret_cast<
                    const
                    quint8 *>(src) + offset));
        offset += static_cast<size_t>(1);
        PRINT_DBG("        TableType: %d", this->tableType);
        if((this->tableType == 0) || (this->tableType == 2))
        {
            checkOffsetLengthSize(offset, static_cast<size_t>(2), srcSize);
            this->minTableWidth =
                getSignedWord(reinterpret_cast<const void *>(reinterpret_cast<
                        const
                        quint8 *>(src) + offset));
            offset += static_cast<size_t>(2);
            PRINT_DBG("        Min table width: %d", this->minTableWidth);
        }
        else
        {
        ...
        for(quint8 i = 0; i < this->numberOfColumns; i++)
        {
            ...
            ci.gapWidth =
                getSignedWord(reinterpret_cast<const void *>(reinterpret_cast<
                        ...
                        quint8 *>(src) + offset));
            offset += static_cast<size_t>(2);
            PRINT_DBG("        Gap width: %d", ci.gapWidth);
            checkOffsetLengthSize(offset, static_cast<size_t>(2), srcSize);
            ci.colWidth =
                getSignedWord(reinterpret_cast<const void *>(reinterpret_cast<
                        const
        ...
        while(offset < srcSize)
        {
            try
            {
                ...
                checkOffsetLengthSize(offset, static_cast<size_t>(2), srcSize);
                paragraphInfo.column =
                    getSignedWord(reinterpret_cast<const void *>(
                        reinterpret_cast<
                            const quint8 *>(src) + offset));
                offset += static_cast<size_t>(2);
                PRINT_DBG("        Column: %d", paragraphInfo.column);
                if(paragraphInfo.column == -1)
                {
                    break;
                }
                checkOffsetLengthSize(offset, static_cast<size_t>(2), srcSize);
                qint16 unknownShort =
                    getSignedWord(reinterpret_cast<const void *>(
                        reinterpret_cast<
                            const quint8 *>(src) + offset));
                offset += static_cast<size_t>(2);
                PRINT_DBG("        Unknown short: %d", unknownShort);
                checkOffsetLengthSize(offset, static_cast<size_t>(1), srcSize);
                qint8 unknownChar =
                    getSignedByte(reinterpret_cast<const void *>(
                        reinterpret_cast<
                            const quint8 *>(src) + offset));
                offset += static_cast<size_t>(1);
                PRINT_DBG("        UnknownChar: %d", unknownChar);
                checkOffsetLengthSize(offset, static_cast<size_t>(1), srcSize);
                quint8 unknownUnsignedByte =
                    getUnsignedByte(reinterpret_cast<const void *>(
                        reinterpret_cast<
                            const quint8 *>(src) + offset));
                offset += static_cast<size_t>(1);
                PRINT_DBG("        Unknown unsigned byte: %d",
                    unknownUnsignedByte);
                checkOffsetLengthSize(offset, static_cast<size_t>(1), srcSize);
                ...
                if(isUnknownPresent)
                {
                    qint32 unknownSignedDWord = getCompressedSignedDWord(src,
                        ...
                        bytesRead);
                    offset += bytesRead;
                    PRINT_DBG("        Unknown signed dword: %d",
                        unknownSignedDWord);
                }
                if(paragraphInfo.isSpacingAbovePresent)
                {
                    paragraphInfo.spacingAbove = getCompressedSignedWord(src,
                        ...
                        bytesRead);
                    offset += bytesRead;
                    PRINT_DBG("        Spacing above: %d",
                        paragraphInfo.spacingAbove);
                }
                else
                {
                ...
                if(paragraphInfo.isSpacingBelowPresent)
                {
                    paragraphInfo.spacingBelow = getCompressedSignedWord(src,
                        ...
                        bytesRead);
                    offset += bytesRead;
                    PRINT_DBG("        Spacing below: %d",
                        paragraphInfo.spacingBelow);
                }
                else
                {
                ...
                if(paragraphInfo.isSpacingLinesPresent)
                {
                    paragraphInfo.spacingLines = getCompressedSignedWord(src,
                        ...
                        bytesRead);
                    offset += bytesRead;
                    PRINT_DBG("        Spacing lines: %d",
                        paragraphInfo.spacingLines);
                }
                else
                {
                ...
                if(paragraphInfo.isLeftIndentPresent)
                {
                    paragraphInfo.leftIndent = getCompressedSignedWord(src,
                        ...
                        bytesRead);
                    offset += bytesRead;
                    PRINT_DBG("        Left indent: %d",
                        paragraphInfo.leftIndent);
                }
                else
                {
                ...
                if(paragraphInfo.isRightIndentPresent)
                {
                    paragraphInfo.rightIndent = getCompressedSignedWord(src,
                        ...
                        bytesRead);
                    offset += bytesRead;
                    PRINT_DBG("        Right indent: %d",
                        paragraphInfo.rightIndent);
                }
                else
                {
                ...
                if(paragraphInfo.isTabinfoPresent)
                {
                    paragraphInfo.tabInfo.numberOfTabStops =
                        getCompressedSignedWord(src, offset, srcSize, bytesRead);
                    offset += bytesRead;
                    PRINT_DBG("        Number of tab stops: %d",
                        paragraphInfo.tabInfo.numberOfTabStops);
                    for(qint16 i = 0;
                        i < paragraphInfo.tabInfo.numberOfTabStops; i++)
                    {
                        Tab tab;
                ...
                while(hasMoreCommands)
                {
                    ...
                    case VFLD_NUMBER:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new VFLDNumberCommand(vfldNumber))));
                    ...
                    case DTYPE_NUMBER:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new DTypeNumberCommand(dtypeNumber))));
                    ...
                    case FONT_NUMBER:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new FontNumberCommand(fontNumber))));
                    ...
                    case TYPE_86:
                    {
                        ...
                        case EMBEDDED_PICTURE_DATA_TYPE_3:
                        {
                            checkOffsetLengthSize(offset,
                                        static_cast<size_t>(2),
                                        srcSize);
                            qint16 pictureIsEmbedded = getSignedWord(
                            ...
                            {
                                checkOffsetLengthSize(offset,
                                            static_cast<size_t>(2), srcSize);
                                qint16 pictureNumber = getSignedWord(
                                            reinterpret_cast<const void *>(
                                                reinterpret_cast<const quint8 *>(
                                                    src) +
                                                offset));
                        ...
                        case EMBEDDED_OBJECT_DATA:
                        {
                            checkOffsetLengthSize(offset,
                                        ...
                                        srcSize);
                            qint16 unknown1 =
                                        getSignedWord(reinterpret_cast<const
                                            void *>(
                                            reinterpret_cast<const quint8 *>(
                                                src) +
                                            offset));
                            ...
                            checkOffsetLengthSize(offset,
                                        static_cast<size_t>(pictureSize - 6),
                                        srcSize);
                            QString embedded = readFixedLengthStringFromBuffer(
                                        src,
                                        srcSize,
                                        offset,
                            ...
                            paragraphInfo.commands.append(QSharedPointer<
                                            FormattingCommand>(dynamic_cast<
                                                FormattingCommand *>(new
                                                Type86Subtype5Command(
                                                    pictureSize, embedded))));
                        ...
                        {
                            qint16 numberOfHotspots = getCompressedUnsignedWord(
                                        src,
                                        offset,
                                        srcSize,
                                        bytesRead);
                    ...
                    case MACRO_COMMAND:
                    {
                        ...
                        offset += static_cast<size_t>(2);
                        checkOffsetLengthSize(offset,
                                static_cast<size_t>(macroLength), srcSize);
                        QString macroString = readFixedLengthStringFromBuffer(
                                src,
                                srcSize,
                                offset,
                    ...
                    case MACRO_WITHOUT_FONT_CHANGE:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new MacroWithoutFontChangeCommand(
                                            macroLength,
                                            macroString))));
                    ...
                    case POPUP_JUMP_E0:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new PopupJumpE0Command(topicOffset))));
                    }
                    break;
                    ...
                    case TOPIC_JUMP_E1:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new TopicJumpE1Command(topicOffset))));
                    }
                    break;
                    ...
                    case POPUP_JUMP_E2:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new PopupJumpE2Command(topicOffset))));
                    }
                    break;
                    ...
                    case TOPIC_JUMP_E3:
                    {
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new TopicJumpE3Command(topicOffset))));
                    }
                    break;
                    ...
                    case POPUP_JUMP_WITHOUT_FONT_CHANGE:
                    {
                        checkOffsetLengthSize(offset,
                                static_cast<size_t>(TopicOffset::size), srcSize);
                        PRINT_DBG(
                                "        Popup jump without font change command");
                        PRINT_DBG("        TopicOffset:");
                        TopicOffset topicOffset(
                                reinterpret_cast<const void *>(reinterpret_cast
                                        <
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new PopupJumpWithoutFontChangeCommand(
                                            topicOffset))));
                    }
                    break;
                    ...
                    case TOPIC_JUMP_WITHOUT_FONT_CHANGE:
                    {
                        checkOffsetLengthSize(offset,
                                static_cast<size_t>(TopicOffset::size), srcSize);
                        PRINT_DBG(
                                "        Topic jump without font change command");
                        PRINT_DBG("        TopicOffset:");
                        TopicOffset topicOffset(
                                reinterpret_cast<const void *>(reinterpret_cast
                                        <
                        ...
                        paragraphInfo.commands.append(QSharedPointer<
                                    FormattingCommand>(dynamic_cast<
                                        FormattingCommand *>(
                                        new TopicJumpWithoutFontChangeCommand(
                                            topicOffset))));
                    }
                    break;
                    ...
                    case POPUP_JUMP_INTO_EXTERNAL_FILE:
                    {
                        ...
                        {
                            checkOffsetLengthSize(offset,
                                        static_cast<size_t>(TopicOffset::size),
                                        srcSize);
                            PRINT_DBG(
                        ...
                        case 1:
                        {
                            ...
                            checkOffsetLengthSize(offset,
                                        static_cast<size_t>(1),
                                        srcSize);
                            quint8 windowNumber = getUnsignedByte(
                                        reinterpret_cast<const void *>(
                                            reinterpret_cast<const quint8 *>(
                                                src) +
                                            offset));
                        ...
                        case 6:
                        {
                            ...
                            PRINT_DBG("        Window name: %s",
                                        windowname.toLocal8Bit().data());
                            paragraphInfo.commands.append(QSharedPointer<
                                            FormattingCommand>(dynamic_cast<
                                                FormattingCommand *>(new
            ...
            {
                qDebug() << "Incomplete ParagraphInfo";
            }
        }
    }
    else
    ...
    PRINT_DBG("Displayable table loaded successfully");
}

DisplayableTable::DisplayableTable(const DisplayableTable& rhs) :
    DisplayableText(rhs), topicSize(rhs.topicSize), topicLength(rhs.topicLength),
    numberOfColumns(rhs.numberOfColumns), tableType(rhs.tableType),
    minTableWidth(rhs.minTableWidth), columns(), paragraphs(rhs.paragraphs),
    texts(rhs.texts)
{
    PRINT_DBG("Displayable table copy constructor");
}

DisplayableTable::~DisplayableTable()
{
    PRINT_DBG("Displayable table destructor");
}

DisplayableTable & DisplayableTable::operator=(const DisplayableTable & rhs)
{
    PRINT_DBG("Displayable table assignment operator");
    if (this != &rhs)
    {
        DisplayableText::operator =(rhs);
        this->topicSize = rhs.topicSize;
        this->topicLength = rhs.topicLength;
        this->numberOfColumns = rhs.numberOfColumns;
        this->tableType = rhs.tableType;
        this->minTableWidth = rhs.minTableWidth;
...
QList<TopicOffset> DisplayableTable::getHyperlinks() const
{
...
QString DisplayableTable::getHTML(bool &empty) const
{
    QString result = "<tr>";
    int textLength = 0;
    int paraIndex = 0;
    ...
    for(int i = 0; i < this->texts.count(); i++)
    {
        ...
        if(paraIndex < this->paragraphs.count())
        {
            ...
            {
                switch(this->paragraphs.at(paraIndex).commands.at(cmdIndex)->
                    getType())
                {
                case NON_BREAK_HYPHEN:
                    if(!isInColumn)
                    ...
                    if(paraIndex < this->paragraphs.count())
                    {
                                ...
                                this->paragraphs.at(paraIndex).commands.count())
                        {
                            quint32 topicHash =
                                    ...
                                    TopicJumpE1Command>()->getTopicOffset().
                                    getRawValue();
                            result += QString(
                                    "<a style=\"border-bottom: 1px solid; color: green\" href=help://help.local/pages?hash=%1>")
                                    .arg(topicHash);
                            isInLink = true;
                        }
                    }
                    ...
                    if(paraIndex < this->paragraphs.count())
                    {
                                ...
                                this->paragraphs.at(paraIndex).commands.count())
                        {
                            quint32 topicHash =
                                    this->paragraphs.at(paraIndex).commands.at(
                                    cmdIndex).dynamicCast<
                                    TopicJumpE3Command>()->getTopicOffset().
                                    getRawValue();
                            result += QString(
                                    "<a style=\"border-bottom: 1px solid; color: green\" href=help://help.local/pages?hash=%1>")
                                    .arg(topicHash);
