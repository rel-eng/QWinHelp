/* Displayable text new version class definition.
...
#ifndef DISPLAYABLETEXTNEW_H_
#define DISPLAYABLETEXTNEW_H_
...
class DisplayableTextNew : public DisplayableText
{
private:
...
public:
    DisplayableTextNew();
    DisplayableTextNew(const void *src,
        size_t srcSize,
        const void *textSrc,
        size_t textSize,
        QTextCodec *codec);
    DisplayableTextNew(const DisplayableTextNew& rhs);
    virtual ~DisplayableTextNew();
    DisplayableTextNew & operator=(const DisplayableTextNew & rhs);
    virtual QList<TopicOffset> getHyperlinks() const;
    virtual QString getHTML(bool &empty) const;
};

#endif /* DISPLAYABLETEXTNEW_H_ */
